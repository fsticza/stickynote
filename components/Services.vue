<template>
  <article class="services">
    <div class="mb-2">
      <h1 class="highlighted h2">Mit kínálunk</h1>
    </div>
    <div class="row">
      <div class="col-sm-6">
        <ul >
          <li>
            Szervezetek Agilis tranzíciója
            <ul>
              <li>
                Szervezet felmérése (agilis audit)
              </li>
              <li>
                Menedzsment és felső vezetés coacholása és képzése annak érdekében, hogy képesekké váljanak agilis szervezet és működések kialakítására.
              </li>
              <li>
                Csapatok képzése (Scrum/Kanban)
              </li>
              <li>
                Scrum Master felkészítés és coacholás
              </li>
              <li>
                Product owner felkészítés és coacholás
              </li>
              <li>
                Tanuló szervezet kialakítása – folyamatos képzések, mentoring
              </li>
              <li>
                Eszközök bevezetésének támogatása
              </li>
            </ul>
          </li>
        </ul>
      </div>
      <div class="col-sm-6">
        <ul>
          <li>Követelmény elemzési képzés PO-k, BA-k, PM-ek részére.</li>
          <li>Emberi együttműködési képességeket fejlesztő tréning (Soft Skill Suli)</li>
          <li>Termék megalapozás, termék fejlesztés és innovációs tréning.</li>
          <li>Prezentációs tréning</li>
          <li>Testre szabott tréningek a szervezet aktuális igényei szerint.</li>
        </ul>
      </div>
    </div>
  </article>
</template>